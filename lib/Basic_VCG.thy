...
\<close>
  ...
  *)
  ...
  named_theorems_rev vcg_prep_external_drules \<open>Drules to be applied when preparing for external solving\<close>
  
  ML \<open>
    structure VCG_External_Solving = struct
      fun prepare_subgoal_tac ctxt = let
        val dthms = Named_Theorems_Rev.get ctxt @{named_theorems_rev vcg_prep_external_drules}
      in
        REPEAT_DETERM' (dresolve_tac ctxt dthms ORELSE' eresolve_tac ctxt @{thms thin_TrueE conjE impE exE})
      end
    end
  \<close>
