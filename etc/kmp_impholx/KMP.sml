...
val nat_of_integer = KMP.nat_of_integer o Int.fromInt

val integer_of_nat = Int.toInt o KMP.integer_of_nat
...
fun main () = let
  ...
  val duration = Time.toMicroseconds (Time.- (Time.now(), start)) div LargeInt.fromInt n
  
  val _ = println ("Time without I/O (us): " ^ IntInf.toString duration)
  
  val _ = println ("result (" ^ Int.toString n ^ " iterations) = " ^ (case res of NONE => "-1" | SOME res => Int.toString res))
  
  val _ = println ("@("^ Int.toString n ^ ") '"^sorig^"': "^ IntInf.toString duration)
