...
begin
  ...
  lemma hf2hnr:
    ...
    shows "\<forall>x xi. P x \<longrightarrow> hn_refine (hn_ctxt (fst R) x xi ** \<box>) (f$xi) (hn_ctxt (snd R) x xi ** \<box>) S (g$x)"
    using assms
    unfolding hfref_def 
...
\<longleftrightarrow> (\<forall>b bi a ai. P (a,b) \<longrightarrow>
  ...
  lemma hnr_intro_dummy:
    "\<forall>x xi. P x \<longrightarrow> hn_refine (\<Gamma> x xi) (c xi) (\<Gamma>' x xi) R (a x) \<Longrightarrow> \<forall>x xi. P x \<longrightarrow> hn_refine (\<Gamma> x xi ** \<box>) (c xi) (\<Gamma>' x xi ** \<box>) R (a x)" 
    by simp
  ...
  *)
  ...
  proof clarsimp
    ...
    show "llvm_htriple
      (hr_comp R1 R1' a1 c1 ** \<Gamma>)
        (c c1 )
      (\<lambda>r. hn_rel (hr_comp R R') (a a1) r ** (hr_comp R1p R1' a1 c1 ** \<Gamma>'))"
    ...
    proof (goal_cases)
      ...
      note R1 = \<open>(b1, a1) \<in> R1'\<close>
      ...
      with NFB R have [vcg_rules]: 
        "llvm_htriple (R1 b1 c1 ** \<Gamma>) (c c1) (\<lambda>r. hn_rel R (b b1) r ** (R1p b1 c1 ** \<Gamma>'))"
        unfolding hn_refine_alt by auto
      
      note AUX = hn_rel_compI[OF NF R'] hr_compI[OF R1]  
        
      show ?case using 1
        apply vcg
        unfolding ENTAILS_def
        apply (simp)
        apply (intro conj_entails_mono entails_refl AUX)
        done
  ...
  lemma hfref_cons:
    ...
    assumes "\<And>x y. S x y \<turnstile> S' x y"
  ...
  lemma hfsynth_hnr_from_hfI:
    assumes "\<forall>x xi. P x \<and> hfsynth_ID_R (fst R) x \<longrightarrow> hn_refine (hn_ctxt (fst R) x xi ** \<box>) (f$xi) (hn_ctxt (snd R) x xi ** \<box>) S (g$x)"
    shows "(f,g) \<in> [P]\<^sub>a R \<rightarrow> S"
    using assms
    unfolding hfref_def 
    by (auto simp: hn_ctxt_def)
