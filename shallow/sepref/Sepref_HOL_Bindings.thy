...
lemma hn_snat_ops[sepref_fr_rules]:
  ...
  "(uncurry ll_icmp_ule, uncurry (RETURN \<circ>\<circ> (\<le>))) \<in> snat_assn\<^sup>k *\<^sub>a snat_assn\<^sup>k \<rightarrow>\<^sub>a bool1_assn"
  "(uncurry ll_icmp_ult, uncurry (RETURN \<circ>\<circ> (<))) \<in> snat_assn\<^sup>k *\<^sub>a snat_assn\<^sup>k \<rightarrow>\<^sub>a bool1_assn"  
  unfolding op_neq_def
...
prepare_code_thms (LLVM) [llvm_code] example1_def example2_def example1n_def example2n_def  
...
prepare_code_thms (LLVM) [llvm_code] example3_def example3n_def  
export_llvm
