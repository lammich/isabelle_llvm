...
text\<open>The basic lemmas on @{const sublist_at} from the previous chapter together 
  with the Refinement Framework's verification condition generator / solver suffice:\<close>
...
lemma "s \<noteq> [] \<Longrightarrow> naive_algorithm s t \<le> kmp_SPEC s t"
  ...
  apply (refine_vcg
    ...
    WHILEIT_rule[where R="measure (\<lambda>(j,_::nat option). length s - j)"]
                 ...
                 apply (all \<open>clarsimp;(auto; fail)?\<close>)
...
definition "kmp s t \<equiv> do {
  ...
  (_,_,pos) \<leftarrow> WHILEIT (I_outer s t) (\<lambda>(i,j,pos). i + length s \<le> length t \<and> pos=None) (\<lambda>(i,j,pos). do {
    ...
    if pos=None then do {
      ASSERT (j < length s \<and> \<ff> s j \<le> j);
      let i = i + (j - \<ff> s j + 1);
      let j = max 0 (\<ff> s j - 1); \<comment> \<open>\<open>max\<close> not necessary, but indicates that subtraction may actually underflow\<close>
      RETURN (i,j,None)
    } else RETURN (i,j,Some i)
  }) (i,j,pos);
...
lemma kmp_correct: "s \<noteq> []
  ...
  subgoal for i jout j using j_le_\<ff>_le[of j s] by simp
...
definition "kmp1 s t \<equiv> do {
  ...
  (_,_,pos) \<leftarrow> WHILEIT (I_outer s t) (\<lambda>(i,j,pos). i + length s \<le> length t \<and> pos=None) (\<lambda>(i,j,pos). do {
    ...
    if pos=None then do {
      ASSERT (j < length \<ff>s \<and> \<ff>s!j \<le> j);
      let i = i + (j - \<ff>s!j + 1);
...
lemma compute_\<ff>s_correct: "compute_\<ff>s s \<le> compute_\<ff>s_SPEC s"
  ...
  subgoal by (metis Suc_eq_plus1 \<ff>_le border_order.less_irrefl diff_Suc_1 extend_border less_SucI less_Suc_eq_le take_Nil zero_less_Suc)
...
lemma compute_\<ff>s_inner_bounds: 
  ...
  shows "1\<le>j" "i-1 < length s" "j-1 < length s"
...
proof -
  have "compute_butlast_\<ff>s s \<le> \<Down> Id (compute_\<ff>s s')"
    ...
    using assms apply (clarsimp_all simp: in_br_conv)
     ...
     apply (simp add: I_out_cb_def)
     apply (metis One_nat_def compute_\<ff>s_inner_bounds(2-) nth_butlast)
     apply (simp add: I_out_cb_def)
     done
...
definition "kmp2 s t \<equiv> do {
  ...
  ASSERT (length \<ff>s = length s);
  (_,_,pos) \<leftarrow> monadic_WHILEIT (I_outer s t) 
    (\<lambda>(i,j,pos). do { ASSERT(i\<le>length t); RETURN (i + length s \<le> length t \<and> pos=None)} ) 
    (\<lambda>(i,j,pos). do {
      ASSERT (i + length s \<le> length t \<and> pos=None);
      (j,pos) \<leftarrow> monadic_WHILEIT (I_in_na s t i) 
        (\<lambda>(j,pos). do { ASSERT(pos=None \<longrightarrow> i+j<length t \<and> j<length s); RETURN (pos=None \<and> t!(i+j) = s!j) }) 
        (\<lambda>(j,pos). do {
          ASSERT (j<length s);
          let j=j+1;
          if j=length s then RETURN (j,Some i) else RETURN (j,None)
      ...
      if pos=None then do {
        ASSERT (j < length \<ff>s \<and> \<ff>s!j \<le> j);
        let i = i + (j - \<ff>s!j + 1);
        let j = op_nat_sub_ovf (\<ff>s!j) 1; \<comment> \<open>Explicitly select 0-saturating subtraction\<close>
        RETURN (i,j,None)
      } else RETURN (i,j,Some i)
    }) (i,j,pos);
...
(* TODO: Use monadic_WHILEIT also in compute_\<ff>s *)
lemmas in_bounds_aux = compute_\<ff>s_inner_bounds[of "butlast s" for s, simplified]

subsection \<open>Setup\<close>
text \<open>We fix a 64 bit type for string positions,
...
abbreviation "string_assn \<equiv> larray_assn' TYPE(size_t) char_assn"  

text \<open>Also the lookup table is represented as array with length value.\<close>
...
*)
abbreviation "tab_assn \<equiv> larray_assn' TYPE(size_t) size_t_assn"
...
subsection \<open>Refinement of Main Algorithm\<close>
...
text \<open>The following theorem relates the final Imperative HOL algorithm to its specification,
  using, beyond basic HOL concepts
    \<^item> Hoare triples for LLVM;
    \<^item> The assertion @{const larray_assn} to specify array-lists, which we use to represent the input strings of the algorithm;
    \<^item> The @{const sublist_at} function that we defined in section \ref{sec:spec}.
  \<close>
  
theorem kmp_impl_correct:
  "llvm_htriple 
    (larray_assn id_assn s si ** larray_assn id_assn t ti ** \<up>(length s + length t < max_snat 64))
    (kmp_impl si ti)
    (\<lambda>i. larray_assn id_assn s si ** larray_assn id_assn t ti 
    ...
    "
proof -
  have 1: "snat_option_assn' TYPE(size_t) a c = \<up>(if c=-1 then a = None else a = Some (unat c) \<and> snat_invar c)" for a c
  ...
  show ?thesis    
   supply R = kmp_impl_correct'[THEN hfrefD, THEN hn_refineD, of "(s,t)" "(si,ti)", simplified]
   apply vcg' 
   done
