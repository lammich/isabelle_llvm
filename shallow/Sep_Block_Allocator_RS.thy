  ...
  begin
    ...
    proof -
      ...
      show "\<exists>p'. p' ## \<alpha> s \<and>
              \<alpha> (put' the_memory\<^sub>L (get' the_memory\<^sub>L s @ [Some b]) s) = p' + \<alpha> s \<and>
              (block (\<alpha>b b) (RP_ADDR (ADDR (length (get' the_memory\<^sub>L s)) this_addr)) \<and>*
               tag (tag_of b) (RP_ADDR (ADDR (length (get' the_memory\<^sub>L s)) this_addr)))
               p'"
