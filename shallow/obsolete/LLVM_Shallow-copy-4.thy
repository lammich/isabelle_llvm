  ...
  *}
  ...
  ML \<open>
    ...
    structure LLC_Preprocessor = struct
      ...
      local
        fun mk_llMT T = let
          val (tname,args) = dest_Type @{typ "unit llM"}
          val args = T::tl args
        in
          Type (tname,args)
        end      
        ...
        fun mk_return x = let 
          val T=fastype_of x 
        in Const (@{const_name \<open>Monad.return\<close>}, T --> mk_llMT T) $ x end      
      
        fun BIND m F nctx = let
          val T = fastype_of m |> LLC_Lib.dest_llM
          val (n,nctx) = Name.variant "tmp" nctx
          val x = Free (n,T)
          
          val f = Term.lambda x (F x nctx)
        in 
          @{mk_term "Monad.bind ?m ?f"}
        end
      
        fun dest_abs' (x,T,t) = let
          val (n,t) = Term.dest_abs (x,T,t)
        in
          (Free (n,T), t)
        end
        
        fun CNV_ABS cnv (Abs xTt) nctx = let
              val (uu,t) = dest_abs' xTt
              val t = CNV_ABS cnv t nctx
              val t = Term.lambda uu t
            in
              t
            end
          | CNV_ABS cnv t nctx = cnv t nctx
        
        
        fun is_mk_const (@{mpat "numeral _"}) = true
          | is_mk_const (Const _) = true
          | is_mk_const (Abs _) = true
          | is_mk_const _ = false
      
        fun is_mk_var (Free _) = true
          | is_mk_var (Var _) = true
          | is_mk_var (Bound _) = true
          | is_mk_var _ = false
              
        fun MK t F nctx = 
          if is_mk_const t orelse is_mk_var t then F t nctx
          else let
              val (c,args) = strip_comb t
              
              fun mk c [] _ = mk_return c
                | mk c (x::xs) nctx = MK x (fn x => mk (c$x) xs) nctx
      
              val t' = mk c args nctx
            in
              BIND t' F nctx
            end
      
        fun CNV @{mpat "return ?f"} nctx = MK f (fn t => K (mk_return t)) nctx
          | CNV (t as Abs _) nctx = CNV_ABS CNV t nctx
          | CNV t nctx = let
              val (c,args) = strip_comb t
              
              fun is_ho t = fastype_of t |> body_type |> LLC_Lib.is_llM
              
              fun mk c [] _ = c
                | mk c (x::xs) nctx = 
                    if is_ho x then mk (c $ CNV x nctx) xs nctx
                    else MK x (fn x => mk (c$x) xs) nctx
        ...
        fun monadify t = CNV t (Term.declare_term_frees t Name.context)
        ...
        fun monadify_conv ctxt = let
            val ctxt = put_simpset HOL_basic_ss ctxt addsimps @{thms Monad.bind_laws}
            val tac = ALLGOALS (simp_tac ctxt)
          in rhs_conv (Refine_Util.f_tac_conv ctxt monadify tac) end
      ...
      in      
        ...
        val cthm_monadify = Conv.fconv_rule o monadify_conv
...
export_llvm foobar: main_exp is main file "exp.ll"

export_llvm (debug) main is main file "test2.ll"
