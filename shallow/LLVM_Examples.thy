...
prepare_code_thms [llvm_code] exp2_def
...
export_llvm (debug) "euclid2" is euclid file "euclid2.ll"

declare [[eta_contract = false]]
thm euclid2.code[no_vars]
...
definition fib :: "64 word \<Rightarrow> 64 word llM" 
  where "fib n \<equiv> REC (\<lambda>fib n. doM { 
...
prepare_code_thms (LLVM) [llvm_code, code] fib_def  
print_theorems

declare [[eta_contract = false]]
thm fib.code(2)[folded fib.code(1), THEN fun_cong, of n]
  
export_llvm fib_0 is fib
...
print_theorems
thm fib'.code(2)[folded fib'.code(1), THEN fun_cong, of n]
export_llvm thms: "fib'_0 :: 64 word \<Rightarrow> 64 word llM" is fib
...
definition "cr_big_al (n::64 word) \<equiv> doM {
...
prepare_code_thms [llvm_code] cr_big_al_def
declare Let_def[llvm_inline]
export_llvm (debug) cr_big_al is "cr_big_al" file "cr_big_al.ll"
...
lemma vcg_prep_lstr_assn[vcg_prep_external_drules]: (* TODO: Need mechanism to recursively prepare pure parts of A! *)
...
lemma pure_part_prepD[vcg_prep_external_drules]: "pure_part (\<Union>*i\<in>I. f i) \<Longrightarrow> \<forall>i\<in>I. pure_part (f i)"
...
definition "qs_partition A lo hi \<equiv> doM {
...
definition "qs_quicksort A lo hi \<equiv> doM {
...
prepare_code_thms [llvm_code] qs_partition_def[unfolded llc_for_range_def]
