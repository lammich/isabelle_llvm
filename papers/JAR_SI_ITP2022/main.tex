...
e.g., \is{\<forall>x. x<4 ==> x<=3} (in Isabelle, one must use \is{-->} here).

Type variables are \is{'a, 'b, ...} and a function types are written curried as \is{'a => 'b => 'c}.
Types can be annotated to any symbol or term, e.g., \is{(x::nat) < 4 ==> x<=3}.
Function application is written as \is{f x}, and function update is \is{f(x:=y)}.
...
Values of the tuple type \is{'a_1 \x ... \x 'a_n} are written as \is{(a_1,...,a_n)}. Lists have type \is{'a list},
...
We also use the list notation \is{[1,2,3]}. Then length of list \is{xs} is \is{|xs|}.
...
Otherwise, we will explicitly write, e.g. \is$ s = s_1 \union s_2  &  s_1 \inter s_2 = {}$. The cardinality of the finite set \is{s} is \is{|s|}.
...
%   Arbitrary recursive programs can be defined via a fixed-point construction~\cite{Kr10}.
  ...
  \begin{example}[Swapping multiple elements]\label{ex:swap_spec}
    We specify an operation to perform multiple swaps. It takes two disjoint sets of indices \is{s_1} and \is{s_2}, and a list \is{xs}. It then swaps each index in \is{s_1} with some index in \is{s_2}.
...
%     xxx: include pointer-preservation for list_set
  ...
  This is a design choice, and a more direct design, using \is{(nat \x nat) list} as intermediate data structure, is certainly possible.
  ...
  and \is{sl_join xs_1 xs_2} joins the elements of two (compatible) lists.
  ...
  Figure~\ref{fig:psort} show our abstract parallel sorting algorithm \is{psort}.
...
% 	node[above] at (axis cs:1950,4.3e7) {Houses};
  ...
  \begin{figure}
    ...
    \plottim{Server, uint64}{data_server_uint64}{xticklabels={},ymax=1000}{\legend{}}
