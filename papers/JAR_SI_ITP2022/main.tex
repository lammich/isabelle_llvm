...
%   to have a guard value there, which saves some indexing comparison
  ...
  \end{lstlisting}
  This instantiates the generic sorting algorithms defined in the \is{pcmp} locale to use \is{lt_idx xs} as comparison function, taking \is{xs} as an extra parameter. To sort our list \is{$is$} of sample indexes into \is{xs}, we use the instantiation of the introsort algorithm: \is{idx.introsort xs $is$}.
