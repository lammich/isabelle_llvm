...
\begin{frame}{Motivation}
  ...
  \begin{itemize}
   \item<+-> Desirable properties of software
    \begin{itemize}
     \item<+-> \onslide<.->{correct}\onslide<+->{ (formally verified)}
     \item<+-> fast
     \item<+-> manageable implementation\only<+->{ and proof} effort
    \end{itemize}
   \item<+-> Choose two!
...
\begin{frame}{Introduction}
  \begin{itemize}
   \item<+-> What does it need to formally verify an algorithm?
    \begin{itemize}
     \item<+-> E.g. maxflow algorithms
  ...
  \only<5-7>{
    \begin{theorem}
      ...
      \begin{itemize}
        ...
        \item $\delta_f$ remains the same, and the number of edges in $g_f$ that lay on a shortest path decreases.
      \end{itemize}
    \end{theorem}
    \uncover<6-7>{
      \begin{proof}
...
\begin{frame}[fragile]{Keeping it Manageable}
  ...
  \end{itemize}
\end{frame}
\begin{frame}{Refinement Architecture (simplified)}
...
\end{frame}
\begin{frame}{Knuth Morris Pratt}
  \begin{tikzpicture}
    \begin{semilogyaxis}[
...
\end{frame}
\begin{frame}{Verified Introsort Algorithm}
  \begin{tikzpicture}
    \begin{semilogyaxis}[
%       xlabel={Benchmark Set},
      xlabel near ticks,
...
%       ytick distance = 10,
      ...
      \only<2> {
        \addplot[color=red,fill=red] coordinates {
        (rev-sorted-end-10,7950)
        (rev-sorted-end-1,9595)
        (sorted-end-.1,12218)
        (almost-sorted-50,10358)
        (random-boolean,4167)
        (organ-pipe,23599)
        (sorted-end-10,14195)
        (equal,3975)
        (rev-sorted-middle-.1,8619)
        (rev-sorted,3544)
        (sorted-middle-1,8138)
        (rev-sorted-middle-10,10667)
        (random,12192)
        (almost-sorted-.1,4910)
        (sorted,4928)
        (rev-sorted-middle-1,8137)
        (sorted-middle-.1,8642)
        (almost-sorted-10,6400)
        (almost-sorted-1,5015)
        (sorted-middle-10,10758)
        (rev-sorted-end-.1,14688)
        (sorted-end-1,25431)
        (random-dup-10,11578)
        };
        \addlegendentry{Isabelle-LLVM};
        ...
        \addplot[color=blue,fill=blue] coordinates {
        (rev-sorted-end-10,7712)
        (rev-sorted-end-1,9233)
        (sorted-end-.1,11285)
        (almost-sorted-50,10103)
        (random-boolean,4195)
        (organ-pipe,20921)
        (sorted-end-10,14007)
        (equal,3939)
        (rev-sorted-middle-.1,8269)
        (rev-sorted,3415)
        (sorted-middle-1,7787)
        (rev-sorted-middle-10,10040)
        (random,11910)
        (almost-sorted-.1,4828)
        (sorted,4785)
        (rev-sorted-middle-1,7852)
        (sorted-middle-.1,8274)
        (almost-sorted-10,6319)
        (almost-sorted-1,4908)
        (sorted-middle-10,10423)
        (rev-sorted-end-.1,13509)
        (sorted-end-1,21205)
        (random-dup-10,11221)
        };
        \addlegendentry{libstd++ (unverified)};
  ...
  \only<1>{Intel Core i7-8665U CPU, 32GiB RAM}\only<2>{AMD Opteron 6176 24 core, 128GiB RAM}.



\end{frame}
\begin{frame}{Isabelle-LLVM: Overview}
...
\begin{frame}{LLVM Semantics}
  \begin{itemize}
   \item<+-> We don't need to formalize all of LLVM!
    \begin{itemize}
     \item just enough to express meaningful programs
     \item abstract away certain details (e.g. in memory model)
    \end{itemize}
   \item<+-> Trade-off
    \begin{itemize}
     \item complexity of semantics vs. trusted steps in code generator
    \end{itemize}
   \item<+-> Our choice:
    \begin{itemize}
     \item rather simple semantics
     \item code generator does some translations
...
\begin{frame}[fragile]{Basics}
  \begin{itemize}
   \item LLVM operations described in state/error monad
    \begin{lstlisting}
   ...
   \item Recursion via fixed-point
    \begin{lstlisting}
...
%     \end{lstlisting}
  ...
  \end{itemize}
\end{frame}
\begin{frame}[fragile]{Shallow Embedding}
...
\begin{frame}[fragile]{Memory Model}
  \begin{itemize}
   \item Inspired by CompCert v1. But with structured values.
    ...
    \begin{itemize}
     \item \is{ADDR i j p} block index, value index, path to value
  ...
  \begin{itemize}
   \item Typeclass \is{llvm_rep}: shallow to deep embedding
    \begin{lstlisting}
    to_val :: 'a => val
    from_val :: val => 'a
  ...
  \begin{itemize}
   \item Shallow pointers carry phantom type
    \begin{lstlisting}
      'a ptr = PTR rptr
    \end{lstlisting}
  ...
  \end{itemize}
\end{frame}
\begin{frame}[fragile]{Example: malloc}
  \begin{lstlisting}
  ...
  \begin{itemize}
   \item Code generator maps \is{ll_malloc} to libc's \is{calloc}.
    \begin{itemize}
     \item out-of-memory: terminate in defined way \is{exit(1)}
...
\begin{frame}[fragile]{Preprocessor}
  \begin{itemize}
   \item Restricted terms accepted by code generator
    \begin{itemize}
   ...
   \item Preprocessor transforms terms into restricted format
    ...
    \begin{itemize}
     \item proves equality (via Isabelle kernel)
      \pause
     \item monomorphization (instantiate polymorphic definitions)
      \pause
     \item flattening of expressions
      \begin{lstlisting}
     ...
     \item tuples
      ...
      \end{lstlisting}
      \pause
     \item Define recursive functions for fixed points
...
\begin{frame}[fragile]{Reasoning about LLVM Programs}
  ...
  \end{itemize}
\end{frame}
\begin{frame}[fragile]{Example: Proving Euclid's Algorithm}
  \small
  \begin{lstlisting}[escapechar=!]
    lemma
...
\begin{frame}{Automatic Refinement}
  \begin{itemize}
   \item<+-> Isabelle Refinement Framework
    \begin{itemize}
     \item supports verification by stepwise refinement
     \item many verified algorithms already exists

    \end{itemize}
   \item<+-> Sepref tool
    \begin{itemize}
     \item refinement from Refinement Framework to imperative program
      \begin{itemize}
       \item already existed for Imperative/HOL
       \item we adapted it for LLVM
      \end{itemize}
     \item existing proofs can be re-used
      \begin{itemize}
       \item need to be amended if they use arbitrary-precision integers

      \end{itemize}
    \end{itemize}
   \item<+-> Collections Framework
    \begin{itemize}
     \item provides data structures
     \item we ported some to LLVM (work in progress)
      \begin{itemize}
       \item dense sets/maps of integers (by array)
       \item heaps, indexed heaps
       \item two-watched-literals for BCP
       \item graphs (by adjacency lists)
       \item ...
...
\end{frame}
\begin{frame}[t,fragile]{Example: Binary Search --- Refinement}
  \begin{lstlisting}
  ...
  \only<2> {
    \is{sint_assn' sz} --- (mathematical) integers by $sz$ bit integers

    \is{snat_assn' sz} --- natural numbers by $sz$ bit integers

    \is{larray_assn' sz e} --- lists by arrays + $sz$-bit length, elements refined by $e$
  }
...
\begin{frame}{Conclusions}
  \begin{itemize}
   \item Fast and verified algorithms
    \begin{itemize}
     \item LLVM code generator
     \item using Refinement Framework
     \item manageable proof overhead
    \end{itemize}
   \item Case studies
    \begin{itemize}
     \item generate really fast, verified code
     \item re-use existing proofs
    \end{itemize}
   \item Current/future work
    \begin{itemize}
     \item more complex algorithms
      \begin{itemize}
       \item promising (preliminary) results for SAT-solver, Prim's algorithm
      \end{itemize}
     \item deeply embedded semantics
     \item unify NRES and HEAP monads
     \item generic Sepref (Imp-HOL, LLVM) $\times$ (nres, nres+time)
