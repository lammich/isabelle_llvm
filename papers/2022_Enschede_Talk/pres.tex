...
\begin{frame}[fragile]{Shallow Embedding Example}
...
\begin{frame}[fragile]{Invariants}
  \begin{itemize}
   \item We prove for \is{llM} (enforced by subtype)
    \begin{itemize}
...
\begin{frame}[fragile]{Code generator}
  \begin{itemize}
   \item<+-> We add \is{llc_par f_1 f_2 x_1 x_2 == f_1 x_1 || f_2 x_2}
    \begin{itemize}
     \item \is{f_1, f_2} must be functions
    \end{itemize}
   \item<+-> Code generator generates
    \begin{itemize}
     \item type casting boilerplate
     \item call to external \is{parallel} function
