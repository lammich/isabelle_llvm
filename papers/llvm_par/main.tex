...
\section{A Back End for LLVM with Parallel Execution}\label{sec:parsem}
  ...
  and \is{bind} sequentially composes the executions, and adds up the access reports.
...
%   , e.g., \is{array xs p * i<|xs|}
  ...
  Abstract programs are shallowly embedded into the nondeterminism error monad \is{'a ne} (cf.\ Section~\ref{sec:monad}).
  They are purely functional, not modifying memory, or differentiating between sequential and parallel execution.
  We define a \emph{refinement ordering} on \is{ne}:
  \begin{lstlisting}
...
%   Arbitrary recursive programs can be defined via a fixed-point construction~\cite{Kr10}.
  ...
  the LLVM program \is{c$_\dagger$} succeeds with \is{r$_\dagger$},
...
%     xxx: include pointer-preservation for list_set
  ...
  the Sepref tool automatically synthesizes an LLVM program from an abstract \is{ne} program.
...
%   to have a guard value there, which saves some indexing comparison
  ...
  }. Moreover, we have a concrete implementation \is{lt$_\dagger$} or the compare function, wrt.\ the refinement assertions
  \is{par$_A$} for the parameter and \is{elem$_A$} for the elements.

  Our sorting algorithm is developed and verified in the context of this locale (to avoid confusion, our presentation has, up to now, just used \is{<} and \is{sorted} instead of \is{lt p} and \is{sorted_wrt (lt p)}).
