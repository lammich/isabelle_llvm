...
# The Introsort Algorithm
  * Combine quicksort, heapsort, and insort to fast $O(n\log n)$ algorithm.
    *<2-> if quicksort recursion too deep, switch to heapsort
    *<3-> use insertion sort for small partitions
      * final insort on array sorted up to threshold
...
#! Verification Methodology: Modularity
  * Specifications for subroutines, e.g. \isai{heapsort <= sort_spec}
...
#![t] Some of the Optimizations
  ...
  * manual tail-recursion optimization
    * replace second \textsc{introsort\_aux} call by loop
...
# Pdqsort: Verification
  *<+-> Similar to introsort, but
    ...
    * insort inside algorithm (rather than final insort)
  *<+-> Verification went mostly smoothly
    * heapsort, and parts of insort could be re-used
...
# Benchmarks: \only<1,3>{Introsort}\only<2,4,5>{Pdqsort} (\only<1,2,5>{64 bit integers}\only<3,4>{strings})\only<1-4>{ (Intel laptop)}\only<5>{ (AMD server)}
  \begin{tikzpicture}
    \begin{axis}[
