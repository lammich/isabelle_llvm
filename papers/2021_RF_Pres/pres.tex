...
\begin{frame}{High-Level Ideas}
  \begin{itemize}
   \item Stepwise refinement
    \begin{itemize}
     \item separates algorithmic ideas from impl details
     \item modularizes programs and proofs
    \end{itemize}
   \item Powerful (interactive) theorem prover
    \begin{itemize}
     \item for ambitious background theory
     \item trustworthy small kernel
     \item Isabelle/HOL: mature prover+IDE, libraries+AFP, sledgehammer
    \end{itemize}
   \item Automation
    \begin{itemize}
     \item tools (e.g. VCG, automatic refinement)
     \item do \high{not} extend TCB
...
\begin{frame}{Example}
  ...
  \begin{algorithmic}
    \Procedure{Kruskal}{$E$}
      \State{$\equiv \gets \text{= on nodes of $E$}$}
      \State{$F \gets \emptyset$}
      \While{$E \neq \emptyset$}
        \State{remove $(u,w,v)$ with minimal $w$ from $E$}
        \If{$\neg u\equiv v$}
          \State{$F \gets F \cup \{(u,w,v)\}$}
          \State{${\equiv} \gets ({\equiv} \cup \{(u,v)\})^{\text{stcl}}$ }
        \EndIf
      \EndWhile
    \EndProcedure
  \end{algorithmic}

%   {\small (Isabelle version at \url{https://www.isa-afp.org/browser_info/current/AFP/Kruskal/Kruskal_Refine.html})}
...
\end{frame}
\begin{frame}[fragile]{Monotonicity}
  \begin{itemize}
   \item Combinators are monotonous, also wrt. data refinement
    \begin{lstlisting}
   ...
   \item Enables syntax-based VCG for refinement
    \begin{itemize}
     \item some heuristics required, e.g., to introduce $R$ in bind-rule

    \end{itemize}
   \item Common refinements:
    \begin{itemize}
     \item specification refinement: \lstinline{c <= spec Q}
     \item structural refinement: \lstinline{combinator c <=\<Down> R combinator c'}
     \item operator refinement: \lstinline{op x <=\<Down> R op x'}
     \item solved by (combined) VCG
...
\begin{frame}[fragile]{Synthesis}
  \begin{itemize}
   \item Given abstract program and concrete data-structures
    \begin{itemize}
     \item synthesize concrete program
     \item using implementations for \is$op$s and \is$spec$s from data-structures

    \end{itemize}
  \end{itemize}
\end{frame}
\renewcommand<>{\State}[1]{\only#2{\beameroriginal{\State}{#1}}}
\newcommand<>{\IState}[1]{\only#2{\State{$\color{red}#1$}}}

\begin{frame}{Example}
  \newcommand{\Rg}{R^{list}_{graph}}
  \newcommand{\Rl}{R^{list}_{set}}
  \newcommand{\Ru}{R^{uf}_{per}}
  \begin{algorithmic}
    \Procedure{Kruskal}{$E$}
      \IState<2>{\Rg~xs~E}
      \State<-2>{$({\equiv}) \gets \text{= on nodes of $E$}$}
      \State<3->{$uf \gets \text{init-uf}~xs$}
      \IState<3>{\Rg~xs~E~~~~\Ru~uf~({\equiv})}
      \State<-3>{$F \gets \emptyset$}\State<4->{$ys \gets []$}
      \IState<4>{\Rg~xs~E~~~~\Ru~uf~({\equiv})~~~~\Rl~ys~F}
      \only<-4>{\While{$E \neq \emptyset$}}\only<5->{\While{$xs \neq []$}}
        \IState<5>{\Rg~xs~E~~~~\Ru~uf~({\equiv})~~~~\Rl~ys~F}
        \State<-5>{remove $(u,w,v)$ with minimal $w$ from $E$}
        \State<6->{$((u,w,v),xs) \gets (hd~xs,tl~xs)$}
        \IState<6>{\Rg~xs~E~~~~\Ru~uf~({\equiv})~~~~\Rl~ys~F}
        \If{$\neg u\equiv v$}
          \State{$F \gets F \cup \{(u,w,v)\}$}
          \State{${\equiv} \gets ({\equiv} \cup \{(u,v)\})^{\text{stcl}}$ }
        \EndIf
...
\begin{frame}{Backends}
  \begin{itemize}
   \item Purely functional
    \begin{itemize}
     \item refine until no SPECs left
     \item then transfer to option monad, and use code generator

    \end{itemize}
   \item Imperative
    \begin{itemize}
     \item synthesis steps use separation logic
     \item and target a heap monad

    \end{itemize}
   \item Targets
    \begin{itemize}
     \item Imperative/HOL: heap monad for Isabelle's code generator
     \item Isabelle LLVM: shallow embedding of LLVM into Isabelle/HOL
